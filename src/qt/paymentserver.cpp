    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Dime address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    if (!refundAddresses.empty()) {
        ...
        if (wallet->GetKeyFromPool(newKey)) {
            std::cout << __PRETTY_FUNCTION__ << ": wallet->GetKeyFromPool(newKey)" << std::endl;
            ...
            std::cout << __PRETTY_FUNCTION__ << ": Error getting refund key, refund_to not set" << std::endl;
        }
    }
    ...
    if (payment.SerializeToArray(serData.data(), length)) {
        QNetworkReply *reply = netManager->post(netRequest, serData);
        connect(reply, SIGNAL(uploadProgress(qint64 bytesSent, qint64 bytesTotal)), this, SLOT(handleUploadProgress(qint64,qint64)));
...
void PaymentServer::handleUploadProgress(qint64 bytesSent, qint64 bytesTotal)
{
    std::cout << __PRETTY_FUNCTION__ << " bytesSent " << bytesSent << " bytesTotal " << bytesTotal << std::endl;
}

void PaymentServer::handleFinished()
{
    std::cout << __PRETTY_FUNCTION__ << std::endl;
}
