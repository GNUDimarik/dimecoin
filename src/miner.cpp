...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DimeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DimeMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        ...
        LogPrintf("Running DimeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DimeMiner terminated\n");
        throw;
    }
}
